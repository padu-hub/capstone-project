...
#define MAX_APP_BUFFER_SIZE          511
...
//  UTIL_SEQ_SetTask((1 << 1), CFG_SEQ_Prio_0);
  ...
  APP_LOG(TS_ON, VLEVEL_M, "T = %u °C, Pressure = %u, humidity = %u, IAQ = %u, Gas = %u, \n\r", (uint8_t)(sensor_data.temperature), (uint8_t)(sensor_data.pressure), (uint8_t)(sensor_data.humidity), (uint8_t)(sensor_data.iaq_score), (uint8_t)(sensor_data.gas_resistance) );
      ...
      if (isMaster == true)
      {
        ...
        if (RxBufferSize > 0)
        {
          if (strncmp((const char *)BufferRx, PONG, sizeof(PONG) - 1) == 0)
          {
            ...
            uint8_t temp_to_send = (uint8_t)(sensor_data.temperature);
            memcpy(BufferTx, &temp_to_send, sizeof(uint8_t));
            //memcpy(BufferTx, &sensor_data, sizeof(sensor_data));
